...
def pair_distance(pair):
    distance = (pair[0][1] - pair [1][1])
    return distance

for key, value in all_g.items():
#    print key, value
    named_genes = zip(key, value[0])

#named_gene_pairs = itercom(named_genes, 2)

#for pair in named_gene_pairs:
#    print pair
#    distances = pair_distance(pair)

#for distance in distances : print distance
#plt.hist(distances, color='red', bins=1000)
#plt.title('Histogram of GC Content', fontsize=12)
#plt.xlabel('')
#plt.ylabel('GC percentage')
#plt.savefig('GC_hist.pdf')
